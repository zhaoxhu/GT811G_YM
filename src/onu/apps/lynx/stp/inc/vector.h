...
typedef struct prio_vector_t {
  ...
  cs_port_id_t   design_port;
  cs_port_id_t   bridge_port;
} PRIO_VECTOR_T;
...
STP_VECT_create (OUT PRIO_VECTOR_T* t,
                 ...
                 IN cs_port_id_t design_port,
                 IN cs_port_id_t bridge_port);
void
...
#ifdef STP_DBG
void
STP_VECT_print (IN cs_int8* title, IN PRIO_VECTOR_T* v);

void
STP_VECT_br_id_print (IN cs_int8 *title, IN BRIDGE_ID* br_id, IN Bool cr);
