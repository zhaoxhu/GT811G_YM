...
void *
stp_in_stpm_create (int vlan_id, cs_int8* name, BITMAP_T* port_bmp, int* err_code)
{
  int port_index;
  register STPM_T* this;

  /* STP_STACK_LOG ("stp_in_stpm_create(%s)", name); */
  ...
  for (port_index = CS_PON_PORT_ID; port_index < max_port; port_index++) {
    if (BitmapGetBit(port_bmp, port_index)) {
      if (! STP_port_create (this, port_index)) {
        /* can't add port :( */
        STP_STACK_LOG ("can't create port %d", (int) port_index);
                    ...
                    UID_STP_MODE_T admin_state)
{
  ...
  /* STP_STACK_LOG ("_stp_in_stpm_enable(%s)", name); */
  this = stpapi_stpm_find (vlan_id);
  ...
  if (! this) { /* it had not yet been created */
    if (STP_ENABLED == admin_state) {/* try to create it */
      STP_STACK_LOG ("implicit create to vlan '%s'", name);
      this = stp_in_stpm_create (vlan_id, name, port_bmp, &err_code);
      if (! this) {
        STP_STACK_LOG ("implicit create to vlan '%s' failed", name);
...
_check_stpm_config (IN UID_STP_CFG_T* uid_cfg)
{
  if (uid_cfg->bridge_priority < MIN_BR_PRIO) {
    STP_STACK_LOG ("%d bridge_priority small", (int) uid_cfg->bridge_priority);
    return STP_Small_Bridge_Priority;
  }

  if (uid_cfg->bridge_priority > MAX_BR_PRIO) {
    STP_STACK_LOG ("%d bridge_priority large", (int) uid_cfg->bridge_priority);
    return STP_Large_Bridge_Priority;
  }

  if (uid_cfg->hello_time < MIN_BR_HELLOT) {
    STP_STACK_LOG ("%d hello_time small", (int) uid_cfg->hello_time);
    return STP_Small_Hello_Time;
  }

  if (uid_cfg->hello_time > MAX_BR_HELLOT) {
    STP_STACK_LOG ("%d hello_time large", (int) uid_cfg->hello_time);
    return STP_Large_Hello_Time;
  }

  if (uid_cfg->max_age < MIN_BR_MAXAGE) {
    STP_STACK_LOG ("%d max_age small", (int) uid_cfg->max_age);
    return STP_Small_Max_Age;
  }

  if (uid_cfg->max_age > MAX_BR_MAXAGE) {
    STP_STACK_LOG ("%d max_age large", (int) uid_cfg->max_age);
    return STP_Large_Max_Age;
  }

  if (uid_cfg->forward_delay < MIN_BR_FWDELAY) {
    STP_STACK_LOG ("%d forward_delay small", (int) uid_cfg->forward_delay);
    return STP_Small_Forward_Delay;
  }

  if (uid_cfg->forward_delay > MAX_BR_FWDELAY) {
    STP_STACK_LOG ("%d forward_delay large", (int) uid_cfg->forward_delay);
...
_stp_in_enable_port_on_stpm (STPM_T* stpm, int port_index, Bool enable)
{
  ...
  if (enable) { /* clear port statistics */ 
    ...
    port->tx_rstp_bpdu_cnt =0;
    port->tx_tcn_bpdu_cnt = 0;
  }  

#ifdef STP_DBG
    STP_STACK_LOG ("Port %s became '%s' adminEdge=%c\r\n",
...
#ifdef STP_DBG
    if (port->pcost->debug) {
      STP_STACK_LOG ("changed operSpeed=%lu\r\n", port->operSpeed);
    }
#endif
...
#ifdef STP_DBG
    if (port->p2p->debug) {
      STP_STACK_LOG ("STP_IN_changed_port_duplex(%s)\r\n", port->port_name);
    }
#endif
...
STP_IN_rx_bpdu (int vlan_id, int port_index, BPDU_T* bpdu, size_t len)
{
  ...
  port = _stpapi_port_find (this, port_index);
  if (! port) {/* port is absent in the stpm :( */
    STP_STACK_LOG ("RX bpdu vlan_id=%d port=%d port is absent in the stpm :(\r\n", (int) vlan_id, (int) port_index);
    RSTP_CRITICAL_PATH_END;
    return STP_Port_Is_Absent_In_The_Vlan;
  }
  ...
  if (port->skip_rx > 0) {
    if (1 == port->skip_rx)
      STP_STACK_LOG ("port %s stop rx skipping\r\n",
                 port->port_name);
    else
      STP_STACK_LOG ("port %s skip rx %d\r\n",
  ...
  if (port->operEdge && ! port->lnkWhile && port->portEnabled) {
        ...
        if (port->topoch->debug) {
                STP_STACK_LOG ("port %s tc=TRUE by operEdge\r\n", port->port_name);
         }
#endif
  ...
  if (port->edge->debug && port->operEdge) {
    STP_STACK_LOG ("port %s not operEdge !\r\n", port->port_name);
  }
#endif
...
STP_IN_one_second (void)
{
  ...
  for (stpm = STP_stpm_get_the_list ( ); stpm; stpm = stpm->next) {
    if (STP_ENABLED == stpm->admin_state) {
      /* STP_STACK_LOG ("STP_IN_one_second vlan_id=%d", (int) stpm->vlan_id); */
                     ...
                     IN UID_STP_CFG_T* uid_cfg)
{
  ...
  register STPM_T* this;
  UID_STP_CFG_T old;
  cs_int32      port_index=0;
  cs_uint32   status=0;
  Bool                enable=0;

           
  /* STP_STACK_LOG ("STP_IN_stpm_set_cfg"); */
  ...
  if (0 != rc) {
    STP_STACK_LOG ("_check_stpm_config failed %d\r\n", (int) rc);
    RSTP_CRITICAL_PATH_END;
    return rc;
  }
  ...
  if ((BR_CFG_STATE & uid_cfg->field_mask) &&
    ...
    if (0 != rc) {
      STP_STACK_LOG ("can't disable rc=%d\r\n", (int) rc);
      RSTP_CRITICAL_PATH_END;
      return rc;
    }
   ...
   /*sync phy port state*/   
          for (port_index=CS_UNI_PORT_ID1 ;port_index < max_port; port_index++) {
              stp_set_port_stp_state( port_index, UID_PORT_FORWARDING);
          }

    if ( ( uid_cfg->field_mask &  (~BR_CFG_STATE))==0 )  {
      RSTP_CRITICAL_PATH_END;
      return 0;
    }
  }
  ...
  if (STP_ENABLED == this->admin_state) {
    if (0 != STP_stpm_check_bridge_priority (this)) {
      ...
      STP_STACK_LOG ("%s\r\n", "STP_stpm_check_bridge_priority failed");
      RSTP_CRITICAL_PATH_END;
            ...
            for (port_index=CS_PON_PORT_ID;port_index < max_port; port_index++) {
                if ( port_index ==CS_PON_PORT_ID){
                 enable=True;
                    ...
                    epon_request_onu_port_link_status_get(context, ONU_DEVICEID_FOR_API, ONU_LLIDPORT_FOR_API, port_index, (cs_sdl_port_link_status_t *)&status);
                      if (status == ONU_PORT_OPER_UP ) {
                          enable=True;
                      }else {
                          enable=False;
                      }
                      }
               STP_IN_enable_port (port_index , enable); 
            }	
         }
     }
...
STP_IN_set_port_cfg (IN int vlan_id, IN UID_STP_PORT_CFG_T* uid_cfg)
{
  ...
  for (port_no = CS_PON_PORT_ID; port_no < max_port; port_no++) {
    if (! BitmapGetBit(&uid_cfg->port_bmp, port_no)) continue;
...
#ifdef STP_DBG
      if (port->edge->debug) {
        STP_STACK_LOG ("port %s is operEdge=%c in STP_IN_set_port_cfg\r\n",
            port->port_name,
            port->operEdge ? 'Y' : 'n');
      }
#endif
    ...
    if (PT_CFG_NON_STP & uid_cfg->field_mask) {
      ...
      if (port->roletrns->debug && port->admin_non_stp != uid_cfg->admin_non_stp) {
        STP_STACK_LOG ("port %s is adminNonStp=%c in STP_IN_set_port_cfg\r\n",
            port->port_name,
     ...
     if (uid_cfg->admin_non_stp==True ) {
        /*sync phy port state*/   
        stp_set_port_stp_state( port_no, UID_PORT_FORWARDING);
     }
                           ...
                           int is_print_err)
{
  ...
  for (port_no = CS_PON_PORT_ID; port_no < max_port; port_no++) {
    if (! BitmapGetBit(ports, port_no)) continue;
