...
STP_roletrns_enter_state (STATE_MACH_T* this)
{
  ...
  switch (this->State) {
...
#ifdef STP_DBG
			if (port->roletrns->debug) 
			   STP_STACK_LOG ("port(%d) set to %s\r\n", port->port_index, "BLOCK_PORT");
#endif
      break;
    case BLOCKED_PORT:
      port->fdWhile = stpm->rootTimes.ForwardDelay;
    ...
    case DESIGNATED_PROPOSE:
      ...
      //cs_printf("Roletrns:  Designated_propose port %d proposing is true\n", port->port_index);
      port->newInfo = True;
#ifdef STP_DBG
      if (port->roletrns->debug)
		  STP_STACK_LOG ("port(%d) set to %s\r\n", port->port_index, "DESIGNATED_PROPOSE");
#endif
      break;
...
#ifdef STP_DBG
      if (port->roletrns->debug)
		  STP_STACK_LOG ("port(%d) set to %s\r\n", port->port_index, "DESIGNATED_SYNCED");
#endif
      break;
...
#ifdef STP_DBG
      if (port->roletrns->debug)
		  STP_STACK_LOG ("port(%d) set to %s\r\n", port->port_index, "DESIGNATED_RETIRED");
#endif
      break;
...
#ifdef STP_DBG
      if (port->roletrns->debug)
		  STP_STACK_LOG ("port(%d) set to %s\r\n", port->port_index, "DESIGNATED_PORT");
#endif
      break;
...
#ifdef STP_DBG
      if (port->roletrns->debug)
		  STP_STACK_LOG ("port(%d) set to %s\r\n", port->port_index, "DESIGNATED_LISTEN");
#endif
      break;
...
#ifdef STP_DBG
      if (port->roletrns->debug)
		  STP_STACK_LOG ("port(%d) set to %s\r\n", port->port_index, "DESIGNATED_LEARN");
#endif
      break;
...
#ifdef STP_DBG
      if (port->roletrns->debug)
		  STP_STACK_LOG ("port(%d) set to %s\r\n", port->port_index, "DESIGNATED_FORWARD");
#endif
      break;
  };
}
...
#ifdef STP_DBG
        if (port->roletrns->debug) {
          STP_STACK_LOG ("hop to BLOCK_PORT role=%d selectedRole=%d\r\n",
