...
STP_port_create (STPM_T* stpm, int port_index)
{
  ...
  temp_ports= stpm->ports;
  STP_NEW_IN_LIST(this, PORT_T, stpm->ports, "port create");

  memset(this, 0, sizeof(PORT_T));
  this->next=temp_ports;

  this->owner = stpm;
  this->machines = NULL;
  this->port_index = port_index;
  this->port_name = stp_strdup((cs_int8 *) STP_OUT_get_port_name (port_index));
  this->uptime = 0;
  ...
  this->timers[iii++] = &this->local_loop_while;    /*shengjianfeng 2008.3.27*/
...
STP_port_init (PORT_T* this, STPM_T* stpm, Bool check_link)
{
 ...
 this->local_loop_while=0;
  ...
  this->local_loop_state=NON_LOOP;
...
int STP_port_trace_state_machine (PORT_T* this, char* mach_name, int enadis, int vlan_id)
{
    ...
    for (stater = this->machines; stater; stater = stater->next) {
        if (! strcmp (mach_name, "all") || ! strcmp (mach_name, stater->name)) {
            ...
            {
                STP_STACK_LOG ("port %s on %s trace %-8s (was %s) now %s",
...
void STP_port_trace_flags (cs_int8* title, PORT_T* this)
{
    ...
    STP_STACK_LOG ("         %-12s: flags=0X%04lx port=%s", title, flag, this->port_name);
#endif
}
