...
#ifdef STP_DBG
  if (port->topoch->debug) 
    STP_STACK_LOG("DETECTED: tcWhile=%d on port %s\r\n", 
...
#ifdef STP_DBG
  if (port->topoch->debug) 
    STP_STACK_LOG("PROPAGATING: tcWhile=%d on port %s\r\n", 
        port->tcWhile, port->port_name);
#endif
#ifdef STRONGLY_SPEC_802_1W
...
#ifdef STP_DBG
  if (port->topoch->debug) 
    STP_STACK_LOG("ACKNOWLEDGED: tcWhile=%d on port %s\r\n", 
        port->tcWhile, port->port_name);
#endif
...
#ifdef STP_DBG
  if (port->topoch->debug) 
    STP_STACK_LOG("NOTIFIED_TCN: tcWhile=%d on port %s\r\n", 
        port->tcWhile, port->port_name);
#endif
      break;
  };
