...
static stp_tx_bpdu_t bpdu_packet  = {
  {/* stp_mac_hdr_t */
...
{ /* 17.19.17 (page 68) & 9.3.2 (page 25) */
  ...
  if (this->owner.port->skip_tx > 0) {
    if (1 == this->owner.port->skip_tx)
      STP_STACK_LOG ("port %s stop tx skipping\r\n",
...
#ifdef STP_DBG
  if (this->debug)
    STP_STACK_LOG ("port %s txTcn\r\n", this->owner.port->port_name);
#endif

     this->owner.port->tx_tcn_bpdu_cnt++;
...
build_config_bpdu (PORT_T* port, Bool set_topo_ack_flag)
{
  ...
  if (port->tcWhile) {
    ...
    if (port->topoch->debug)
      STP_STACK_LOG ("tcWhile=%d =>tx TOLPLOGY_CHANGE_BIT to port %s\r\n",
...
#ifdef STP_DBG
  if (this->debug)
    STP_STACK_LOG ("port %s txConfig flags=0X%lx\r\n",
        port->port_name,
...
{/* 17.19.16 (page 68) & 9.3.3 (page 25) */
  ...
  if (this->owner.port->skip_tx > 0) {
    if (1 == this->owner.port->skip_tx)
      ...
      STP_STACK_LOG ("port %s skip tx %d\r\n",
  ...
  if (port->synced) {
    ...
    if (port->roletrns->debug)
      STP_STACK_LOG ("tx AGREEMENT_BIT to port %s", port->port_name);
#endif
...
#ifdef STP_DBG 
    if (port->roletrns->debug)
      STP_STACK_LOG ("tx PROPOSAL_BIT to port %s", port->port_name);
#endif
  ...
  if (port->forwarding) {
    ...
    if (port->transmit->debug)
      STP_STACK_LOG ("tx FORWARDING BIT to port %s", port->port_name);
#endif
...
#ifdef STP_DBG
  if (this->debug)
    STP_STACK_LOG ("port %s txRstp flags=0X%lx\r\n",
        port->port_name,
        (unsigned long) bpdu_packet.body.flags);
#endif
