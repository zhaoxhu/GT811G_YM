                ...
                PORT_ROLE_T newRole)
{
  ...
  switch (newRole) {
    ...
    case NonStpPort:
      ...
      break;
    default:
      STP_STACK_LOG ("%s-%s:port %s => Unknown (%d ?)",
...
#ifdef STP_DBG
      if (port->roletrns->debug)
          STP_STACK_LOG ("updtRootPrio: dm=%d rootTimes.MessageAge=%d on port %s",
...
{               /* 17.19.21 */
  register PORT_T* port;
  register STPM_T* stpm;
  cs_port_id_t old_root_port; /* for tracing of root port changing */
...
setSelectedBridge (STPM_T* this)
{
  ...
  for (port = this->ports; port; port = port->next) {
    if (port->reselect) {
#ifdef STP_DBG
      STP_STACK_LOG ("setSelectedBridge: TRUE=reselect on port %s\r\n", port->port_name);
#endif
      return False;
    }
  }
...
STP_rolesel_check_conditions (STATE_MACH_T* s)
{
  ...
  if (stpm != NULL) {
	  ...
	  if(handler != NULL)
		  handler(0xffff , NULL, 0); /* for bug 7875*/
  }
  ...
  switch (s->State) {
    ...
    case ROLE_SELECTION:
      ...
      for (port = stpm->ports; port; port = port->next) {
        if (port->reselect) {
          /* STP_STACK_LOG ("reselect on port %s", port->port_name); */
          return STP_hop_2_state (s, ROLE_SELECTION);
        }
      }
      break;
...
STP_rolesel_update_stpm (STPM_T* this)
{
  ...
  STP_STACK_LOG ("%s", "??? STP_rolesel_update_stpm ???");
