...
flashiodev_init( struct cyg_devtab_entry *tab )
{
    ...
    /* Get the JFFS2 partition info */
    extern char* jffs2_flash_base_address;
    extern cyg_uint32 jffs2_flash_offset;
    extern cyg_uint32 jffs2_flash_length;
    extern void get_jffs2_partition_info (void);
    extern void flash_writer_setup(void);


    /* Move mack to cmn/main.c */
    /* Get partition info from flash */
    flash_writer_setup();

    /* Get jffs2 partition info */
    get_jffs2_partition_info();

    if (jffs2_flash_base_address == 0x0) {
        diag_printf("ERROR: JFFS2 Flash file system init failed.\n");
        diag_printf("ERROR: JFFS2 base address set to 0x0.\n");
        return false;
    }
    ...
    if ( stat == 0 ) {
        ...
        //dev->end = (char *)flash_info.start + CYGNUM_IO_FLASH_BLOCK_OFFSET_1 + CYGNUM_IO_FLASH_BLOCK_LENGTH_1;
        dev->start = (char *)flash_info.start + jffs2_flash_offset;
        dev->end = (char *)flash_info.start + jffs2_flash_offset + jffs2_flash_length;
#endif
        return true;
    } else
        return false;
