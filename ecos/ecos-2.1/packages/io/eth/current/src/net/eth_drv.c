...
eth_drv_recv(struct eth_drv_sc *sc, int total_len)
{
    ...
    //diag_printf("bsd_tcpip: eth_drv_recv total_len %d\n", total_len);
    ...
    total_len -= sizeof(*eh);
    //diag_printf("bsd_tcpip: total_len %d\n", total_len);
    ...
    if (m) {
        ...
        //diag_printf("bsd_tcpip: m_pkthdr.len = total_len %d\n", total_len);
...
#endif
                //diag_printf("bsd_tcpip to call funs->deliver(sc)\n");
            (*sc->funs->deliver)(sc);
        }
    }
}
