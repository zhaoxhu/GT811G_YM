...
#undef ticks
#if (CYGNUM_HAL_RTC_DENOMINATOR==1000)
int hz = 1000;
int tick = 1000;  // usec per "tick"
#else
int hz = 100;
...
db_show_radix_node(struct radix_node *rn, void *w)
{
    ...
    printf(" flags=%ld refcnt=%ld use=%ld \n",
...
extern int rn_walktree(struct radix_node_head *, walktree_f_t *f, void *w);
