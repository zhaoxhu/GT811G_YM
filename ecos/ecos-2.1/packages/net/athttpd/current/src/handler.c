...
cyg_httpd_find_ires(char *path_name)
{
    ...
    {
        ATHTTPD_DEBUG_2("find ires path_name %s, f_pname %s\n", path_name, entry->f_pname);
        if (!strcasecmp((char*)path_name, entry->f_pname))
        {
            ATHTTPD_DEBUG_2("found ires entry %p\n", entry);
            return entry;
        }
        entry++;
    }
...
cyg_httpd_find_handler(void)
{
    ...
    {
        ATHTTPD_DEBUG_2("find handler url %s, path %s\n", httpstate.url, entry->path);

        if (strcmp((const char*)httpstate.url, entry->path) == 0)
        {
#if 1
            /*diag_printf("Handle web %s \n",(const char*)httpstate.url);*/
            if(strcmp((const char*)httpstate.url, "/cgi-bin/web_timeout.cgi"))
            {
                /*Need sync the web timeout interval with js and cgi.Contact to DingYi if any good idea*/
                extern unsigned char cgi_web_timeout_check(void);
                cgi_web_timeout_check();
            }
#endif
            ATHTTPD_DEBUG_1("found handler %p\n", entry->h);
            return entry->h;
        }
        entry++;
    }
