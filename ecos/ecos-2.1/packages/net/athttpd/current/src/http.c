...
const char *month_of_year[12] = {"Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"};
const char *home_pages[] = {"index.html",   "index.htm", "default.html", "home.html", "config_uid_passwd_ip.html"};
...
CYG_HTTPD_MIME_TABLE_ENTRY(hal_htm_entry,   "htm",  "text/html; charset=utf-8");
CYG_HTTPD_MIME_TABLE_ENTRY(hal_html_entry,  "html", "text/html; charset=utf-8");
...
CYG_HTTPD_MIME_TABLE_ENTRY(hal_txt_entry,    "txt",   "text/plain");
CYG_HTTPD_MIME_TABLE_ENTRY(hal_bin_entry,    "bin",   "application/octet-stream");
...
cyg_httpd_send_error(cyg_int32 err_type)
{
    ...
    ATHTTPD_DEBUG_2("Sending error: %d\n", err_type);
...
cyg_httpd_send_file(char *name)
{
    ...
    if (rc < 0)
    {
        ...
        if (p != NULL)
        {
            ATHTTPD_DEBUG_2("Sending Internal Resource: %s\n", name);
    ...
    {
        ATHTTPD_DEBUG_2("Sending header only for URL: %s\n", file_name);
    ...
    ATHTTPD_DEBUG_2("Sending file: %s\n", file_name);
    ...
    while (bytes_written < sp.st_size)
    {
        ...
        len = cyg_httpd_write(httpstate.outbuffer, bread);
        if(len < 0){
            printf("ERROR: %s\n", strerror(errno));
            break;
        }
...
cyg_httpd_format_header(void)
{
    ...
    char cookie[64] = {0};
    strcat(httpstate.outbuffer, "Set-Cookie: ");
    
    cyg_auth_cookie_get(cookie);

    strcat(httpstate.outbuffer, cookie);
     strcat(httpstate.outbuffer, "\r\n");
...
cyg_httpd_handle_method_GET(void)
{
    ...
    if (h != 0)
    {
        ATHTTPD_DEBUG_1("GET Request handler %p\n", h);
        h(&httpstate);
        return;
    }

    ATHTTPD_DEBUG_1("GET Request file %s from FS\n", httpstate.url);
...
cyg_httpd_parse_POST(char* p)
{
    ...
    ATHTTPD_DEBUG_2("POST Request URL: %s\n", httpstate.url);
...
cyg_httpd_parse_GET(char* p)
{
    ...
    {
        ATHTTPD_DEBUG_2("GET Request URL: %s\n", httpstate.url);
    }
    else
    {
        ATHTTPD_DEBUG_2("HEAD Request URL: %s\n", httpstate.url);
    }
...
cyg_httpd_process_method(void)
{
    ...
    while (*p != '\0')
    {
        ...
        cyg_httpd_auth_table_entry* auth = cyg_httpd_is_authenticated_private(httpstate.url);

        if (auth != 0)
        {          
            if(!auth)
                httpstate.needs_auth=cyg_httpd_auth_entry_from_path(httpstate.url);
