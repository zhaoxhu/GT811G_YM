...
#ifdef CYGPKG_LWIP_IPV4_SUPPORT
static int lwip_writev(int fd, cyg_iovec *iovec_bufs, int count)
{
    int _idx = 0;
    int size_w = 0;
    
    if(!iovec_bufs) {
        diag_printf("%s error, fd %d, %d\n", __func__, fd, __LINE__);
        return -1;
    }
    for (_idx = 0; _idx < count; _idx++) {
        size_w+=lwip_send(fd, iovec_bufs[_idx].iov_base,iovec_bufs[_idx].iov_len, 0);
    }
...
cyg_httpd_writev(cyg_iovec *iovec_bufs, int count)
{
    ...
    //TODO : cortina 
	#ifdef CYGPKG_LWIP_IPV4_SUPPORT
    ssize_t sent = lwip_writev(httpstate.sockets[httpstate.client_index].descriptor,
                          iovec_bufs,
                          count);
	#else
    ...
    if (sent != buf_len)
    {
        ATHTTPD_DEBUG_1("writev() did not send out all bytes (%ld of %ld)\n",
                    sent,
                    buf_len);
    }
    /*diag_printf("%s, write fd %d, data len %d, count %d, buf len %d\n", 
        __func__, httpstate.sockets[httpstate.client_index].descriptor, sent
        , count, buf_len);
        */
    return sent;
}
...
cyg_httpd_write_chunked(char* buf, int len)
{
    if (len == 0)
         return 0;
    ATHTTPD_DEBUG_1("write chunked, len: %d.\n", len);
...
cyg_httpd_process_request(cyg_int32 index)
{
    ...
    ATHTTPD_DEBUG_1("handle req: idx %d, desc %d\n", index, descr);
    ...
    do
    {
        ...
        if (len == 0)
        {
            ...
            ATHTTPD_DEBUG_1("EOF received on descriptor: %d. Closing it.\n", descr);
            return;
        }
        ...
        if (len < 0)
        {
            ...
            ATHTTPD_DEBUG_1("ERROR reading from socket. read() returned: %d\n", httpstate.inbuffer_len);
            return;
        }

        ATHTTPD_DEBUG_1("descr %d received data, len %d\n", descr, len);
        ...
        while (httpstate.request_end != 0)
        {
            ...
            if (httpstate.mode & CYG_HTTPD_MODE_CLOSE_CONN)
            {
                ...
                //TODO cortina
                    FD_CLR(descr, &httpstate.rfds);
                    httpstate.sockets[index].descriptor = 0;
                    //diag_printf("%s, close fd %d , %d\n", __func__, descr, __LINE__);
                }                
                ATHTTPD_DEBUG_1("descr %d shut down\n", descr);
            }
...
cyg_httpd_handle_new_connection(cyg_int32 listener)
{
    ...
    ATHTTPD_DEBUG_1("Opening descriptor: %d\n", fd_client);
...
cyg_httpd_close_unused_sockets(cyg_int32 listener)
{
    ...
    ATHTTPD_DEBUG_1("Garbage collector called\r\n");
    ...
    for (i = 0; i < CYGPKG_NET_MAXSOCKETS; i++)
    {
        if (httpstate.sockets[i].descriptor != 0)
        {
            ...
            {
                ATHTTPD_DEBUG_1("Closing descriptor: %d\n", httpstate.sockets[i].descriptor);
                shutdown(httpstate.sockets[i].descriptor, SHUT_WR);
                {
                //TODO cortina
                    FD_CLR(httpstate.sockets[i].descriptor, &httpstate.rfds);
                    //diag_printf("%s, close fd %d , %d\n", __func__, httpstate.sockets[i].descriptor, __LINE__);
...
//    init_all_network_interfaces();
    ...
    ATHTTPD_DEBUG_0("Tcl interpreter has been initialized...\n");
#endif
...
//    ct_web_init();
    ...
    */
    ...
    ATHTTPD_DEBUG_0("Web server started and listening...\n");
    ...
    while (1)
    {
        ...
        if (rc > 0)
        {
            ...
            {
                ATHTTPD_DEBUG_1("request from listener %d\n", listener);
        ...
        else
        {
            ...
            //diag_printf("%s, Error during select() %d\n", __func__, __LINE__);
        }
    }
}
