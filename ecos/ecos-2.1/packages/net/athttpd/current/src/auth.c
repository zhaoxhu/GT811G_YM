...
cyg_auth_handler  g_do_auth = NULL;
cyg_get_cookie_handler  g_get_cookie_str = NULL;
cyg_get_access_status_handler g_get_access_status = NULL;
        ...
        if (entry->auth_mode == CYG_HTTPD_AUTH_BASIC)
        {
            ...
            else
            {
                *colon = '\0'; // Crypto now has the username.
                ...
                diag_printf("t un=%s, ps=%s\n", cyg_httpd_md5_digest, colon+1);

                entry = cyg_httpd_auth_entry_from_domain(cyg_httpd_md5_digest);
                cyg_httpd_current_authName = entry->auth_domainname;

               diag_printf("t old un=%s, old ps=%s\n", entry->auth_username, entry->auth_password);
...
cyg_httpd_auth_table_entry*
cyg_httpd_is_authenticated_private(char* fname)
{
    // Let's check if the directory access needs authorization. The
    //  authentication is done on the directory name.
    cyg_httpd_auth_table_entry* entry =
    ...
    if (entry != 0)
    {
        if (entry->auth_mode == CYG_HTTPD_AUTH_BASIC)
        {
            ...
            else
            {
                *colon = '\0'; // Crypto now has the username.
                if(g_do_auth != NULL){
                    if(g_do_auth(cyg_httpd_md5_digest,++colon) == 0 )
                        return (httpstate.needs_auth = entry);
                }
            }
        }
...
void cyg_auth_handlers_reg(
        cyg_auth_handler auth, 
        cyg_get_cookie_handler get_cookie,
        cyg_get_access_status_handler get_access_status)
{
    g_do_auth = auth; 
    g_get_cookie_str = get_cookie;
    g_get_access_status = get_access_status;
}

void cyg_auth_cookie_get(char * str)
{
    if(g_get_cookie_str != NULL)
        g_get_cookie_str(str);
}

unsigned char cyg_auth_access_status_get()
{
    if(g_get_access_status != NULL){
        return g_get_access_status();
    }
    return 1;
}
