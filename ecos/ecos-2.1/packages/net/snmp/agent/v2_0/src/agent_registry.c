...
{
  extern unsigned char snmp_ro_community [256];
  extern unsigned char snmp_rw_community [256];
  struct view_parameters view_parms;
  view_parms.pdu = pdu;
  view_parms.name = 0;
  view_parms.namelen = 0;
  view_parms.errorcode = 0;
/*
  *	add private check here
  */
 // diag_printf("check pdu community %s len is %d\n",pdu->community,pdu->community_len);
#if 1
if((strcasecmp(pdu->community, snmp_ro_community) != 0 ) && (strcasecmp(pdu->community, snmp_rw_community) != 0))
	{
	//diag_printf("community is deny. cmp_ro is%d,cmp_rw is %d\n",strcasecmp(pdu->community, snmp_ro_community),strcasecmp(pdu->community, snmp_rw_community));
	//diag_printf("community is deny.pdu->community %p is %s,snmp_ro_com is %s,snmp_rw_com is %s\n",pdu->community,pdu->community,snmp_ro_community,snmp_rw_community);
	return 1;// deny access
	}
if((strcasecmp(pdu->community, snmp_ro_community) == 0 ) && (pdu->command == SNMP_MSG_SET) && (strcasecmp(pdu->community, snmp_rw_community) != 0 ))
	{
	//diag_printf("ro_community can't use for SET\n");
	return 1;//// deny access
	}
