...
unsigned char snmp_trap_dest_ip[4];
unsigned char snmp_trap_community[256];
bool snmp_trap_enable=TRUE;
unsigned char snmp_ro_community [256];
unsigned char snmp_rw_community [256];
...
void set_trap_session_community(unsigned char *p_comm,unsigned char *p_ip)
{
	strcpy(trapsink.community,p_comm);
	memcpy(trapsink.ip,p_ip,4);

	if (trapsink.ip != 0) {
	        sprintf (sink, "%d.%d.%d.%d",
	                 trapsink.ip[0], trapsink.ip[1], trapsink.ip[2], trapsink.ip[3]);
	}
	...
	if (create_trap_session (sink,
                ...
                SNMP_VERSION_1,
                SNMP_MSG_TRAP) == 0) {
                snmp_log(LOG_INFO,"Creation of v1 trap session failed \n");
        }
	snmp_log(LOG_INFO,"reinit_trap_session with ip %d.%d.%d.%d community %s \n",
		trapsink.ip[0], trapsink.ip[1], trapsink.ip[2], trapsink.ip[3],trapsink.community);

	
	//diag_printf("trapsink.community is %s \n",trapsink.community);
}
void get_trap_session_community(unsigned char *p_comm,unsigned char *p_ip)
{
	strcpy(p_comm,trapsink.community);
	memcpy(p_ip,   trapsink.ip, 4);
	//diag_printf("trapsink.community is %s \n",trapsink.community);
}

void set_snmp_community(unsigned char *p_ro_comm,unsigned char *p_rw_comm)
{
	strcpy(snmp_ro_community,p_ro_comm);
	strcpy( snmp_rw_community,p_rw_comm);	
	//diag_printf("set  ro %s  rw %s\n",snmp_ro_community,snmp_rw_community);
	
}
void get_snmp_community(unsigned char *p_ro_comm,unsigned char *p_rw_comm)
{
	strcpy(p_ro_comm, snmp_ro_community);
	strcpy(p_rw_comm, snmp_rw_community);	
	//diag_printf("get  ro %s  rw %s\n",p_ro_comm,p_rw_comm);
}

void 
init_trap_session()
{
	trapsink.port= SNMP_TRAP_PORT;
	trapsink.ip[0]=snmp_trap_dest_ip[0];
	trapsink.ip[1]=snmp_trap_dest_ip[1];
	trapsink.ip[2]=snmp_trap_dest_ip[2];
	trapsink.ip[3]=snmp_trap_dest_ip[3];

	//strcpy(trapsink.community,"public");
	//strcpy(trapsink.community,snmp_trap_community);
	...
	snmp_log(LOG_INFO,"init_trap_session \n");
...
#define stderr_log 1
 ...
 //   send_easy_trap (0, 0);
	...
	/*init ro/rw commnity*/
	strcpy(snmp_ro_community,   "public");
	strcpy(snmp_rw_community,   "public");
	...
	init_trap_session();
      ...
      /* send coldstart trap via snmptrap(1) if possible */
    	 send_easy_trap (0, 0);	
        diag_printf("try send_easy_trap \n");
...
//end del by libo
	...
	if (count > 0){
		...
		cs_thread_delay(300);
		diag_printf("bad socket restart receive\n",count);
		continue;
		return -1;
	    default:
