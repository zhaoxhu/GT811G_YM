	   ...
	   size_t length)
{
	...
	if (community_length) {
		...
		// modified by wujm@2016-04-12
	    // snmp_comstr_parse创建community时
		// community_length不包括字符串结尾'\0',导致check_access()出错
		// 所以malloc时+1
	    pdu->community = (u_char *)malloc(community_length+1);
	    memmove(pdu->community, community, community_length);
		pdu->community[community_length] = 0;
	}
